...
**Read the plant before you read a silence.** Where the arms agreed and no seat cited the plant in the intact arm, the run is **void** and licenses nothing: it has not shown it could detect a signal of that shape, so its silences say nothing. A run that separated on the text under test has no silence to read and stands on what it returned. The control certifies the run, not the item — an item no seat treated as a live candidate is uninformative rather than null, and is rebuilt.
...
**Where a plant must be scored, or the run would license removing the text under test: `references/licensing-a-deletion.md`** — what counts as a seat having cited text, and what a null has to look like before text may go. Load it whenever you score a plant, and always where a deletion is the point.
