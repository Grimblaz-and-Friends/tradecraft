...
**Neither limb fires on a question whose answer you are simply going to consume.** Where nothing you write down rests on it, going and finding out is ordinary work; what makes the paradigm case below a spike is not that the guard is expensive to run, but that an artifact is affirmed and handed off on the strength of it.
...
Where the executor is a reader, the run you need is a fresh session. Four properties, and each is what keeps the result evidence rather than one more opinion:
