...
**Loaded when** you are appending a review's row to a review index. Copy the row whole and consider every key; `../references/the-record.md` says what the row is for and what it may not carry.
...
- `staffing`: a seat that failed unreplaced is said in the value as a panel that was short wherever omission would otherwise misdescribe the run; configured automation never reaches staffing.
