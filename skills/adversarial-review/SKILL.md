...
- **A remedy reproduces the defect class it closes.** Re-read a correction's own source *before* writing the correction, and hold the repair to the standard it enforces. This repo's first panel review produced three in one cycle: a regrade declared nonexistent a tally its own citation contained, a quote-fidelity repair shipped a freshly spliced quote, and a verbatim-quote repair left a false quantifier wrapped around the now-correct quote (ledger rows F2, PF7, PF5) — and prose written mid-review to close a finding committed the same false-absolute class the review had already graded high twice (PF4). The predecessor measured the shape at scale: six findings of one review were remedies reproducing their own defect class.
...
- **Scope to everything the fix touched** — every code branch and every edited passage, not the guard named in the finding — **including the review's own bookkeeping**: the merged ledger, the rows written, the commit message, the report. The parent's output is the one surface no seat reviewed, and in this repo's first panel review it shipped a miscounted seat tally that only the post-fix look caught (ledger row PF1). The predecessor's best-documented escape was mutation-tested post-fix and still missed because the scope was pinned to the named guard — and the defect class then recurred three times in the fix's own mirror image, on the implementing PR's own record ([PR #887](https://github.com/Grimblaz/agent-orchestra/pull/887); systemic remedy at [#886](https://github.com/Grimblaz/agent-orchestra/issues/886)).
