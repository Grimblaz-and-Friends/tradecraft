...
- **An absence claim, a universal, a superlative, or a count requires the check that would falsify it** — run it before the sentence ships, or hedge to what the record supports. Unsubstantiated "X never happens" claims were all three false findings in the predecessor's measured worst round, killed by defense with direct reads of the live tree ([#882](https://github.com/Grimblaz/agent-orchestra/issues/882)); in this practice's own reviews, unverified absolutes in newly written prose are where high-severity findings have concentrated ([PR #2](https://github.com/Grimblaz-and-Friends/tradecraft/pull/2)).
...
- **Hold a remedy to the standard it enforces**, and re-read a correction's own source before writing the correction. A repair reproduces the class it closes often enough to check for by default: a regrade that declares a real record nonexistent, a quote repair that ships a freshly spliced quote, a crash fix applied at one call site while its twin sits untouched beside it ([PR #2](https://github.com/Grimblaz-and-Friends/tradecraft/pull/2); the predecessor's [review-exhibits](https://github.com/Grimblaz/agent-orchestra/blob/main/skills/adversarial-review/references/review-exhibits.md) records a panel finding six of them in one review).
...
- **Scope to everything the fix touched** — every code branch and every edited passage, not the guard named in the finding — **including the review's own bookkeeping**: the merged finding list, the defect rows written, the commit message, the report. No seat reviews the parent's output, and a miscounted seat tally has shipped that way ([PR #2](https://github.com/Grimblaz-and-Friends/tradecraft/pull/2)). Order: commit the fixes, write the rows, then look over both — and a defect found in bookkeeping already pushed is dispositioned `recorded`, since rewriting pushed history is not available. This scope elaborates the constitution's "every branch the fix touched"; it does not depart from it. The predecessor's best-documented escape was mutation-tested post-fix and still missed because the scope was pinned to the named guard — and the defect class then recurred three times in the fix's own mirror image, on the implementing PR's own record ([PR #887](https://github.com/Grimblaz/agent-orchestra/pull/887); systemic remedy at [#886](https://github.com/Grimblaz/agent-orchestra/issues/886)).
