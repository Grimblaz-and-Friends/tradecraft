...
**Every dispatch is built in three parts, in this order.** The **shared block** first and nothing before it: the assignment — artifact location, diff base, purpose statement, output format, evidence standards — itself byte-identical across every recipient dispatched against the same review state, and every predecessor stage's output whole, both carried verbatim or by link, never the dispatcher's paraphrase. Then a **dispatcher's note** where there is one, labeled as such and additive; the seat stage takes none, because a cold vantage sits there and aimed attention is what that vantage is defined against. Last the recipient's own identity, its lens brief in the skill's words, and working location where it differs — the only per-recipient content. **Calling attention is lawful; filtering and restating are not.** An author compressing its own charter is an interested-party summary, and where the dispatching session is the author, one that retypes the assignment per recipient decides one recipient at a time what gets reviewed; one literal string cannot drift.
