...
A change that decides or changes behavior, a rule, or a mechanism's public surface settles its implementation brief before building. When the owner affirms that brief's risk-and-lane pair as `ordinary` / `mechanical`, the implementer skips the pre-implementation artifact and its cold settlement because the brief records a determinate correction with no substantive choice left for the builder. Every other brief-backed change writes that **pre-implementation artifact** — the plan, boundaries, and falsifiable checks that demonstrate the brief was understood — and has an independent reader who did not share the implementer's context settle it before the first commit. Purely mechanical work that makes no decision owes neither brief nor artifact, because ceremony pays for decisions and should not tax work that makes none; that no-brief class is distinct from the owner-affirmed mechanical lane.

## 4. Proof and release

Before release, run the executable checks against the tree that will ship; exercise the built result as its consumer would when the changed paths require that use, or record why no use applies; for an owner-affirmed mechanical lane, record its lane exemption even when the changed paths would otherwise require use; resolve every comment from a **connected reviewer**, meaning a configured reviewer that automatically examines the pull request; and record in the pull request which stages the change skipped, bypassed or stalled at and why, or that the expected path ran without a departure. Merging is the owner's decision, because evidence can inform release authority but cannot replace it.
