...
**The implementer first repairs what a repair reaches** — a claim the verdicts found false about the tree is probed and rewritten, a passage of the wrong shape restated; those are the repeated-point rule's own remedies and no ruling substitutes for them. **The holder decides and records every unresolved point inside the affirmed rows.** A question the record or precedent can answer goes first to the assigned coordinating session, or through the holder's recorded check where none is assigned. The owner is reached only where the answer changes an affirmed row or the expected saving exceeds the ask's cost, through the one argued form with that cost stated. **The repairs and recorded cap resolution settle the artifact** — the label states **settled by the recorded cap resolution**, naming any qualifying owner ruling, never the word bare, so no stage inherits it as a reading a seat affirmed. **Against the repeated-point rule's third response the cap governs**: the work proceeds on the recommendation below the cap; at the cap, the holder records the resolution before the artifact can be built from. **An amended implementation brief is a new term, and the count starts again there** — the artifact is settled against it exactly as it always was, a fresh dispatch included, because two verdicts against the implementation brief it replaced judged something else. Exhausted rounds alone are not a third owner door.
