...
Dispatch ownership and judging-seat staffing are in `../references/the-stretch.md`.
...
**This section is the whole of what every cold dispatch takes from this file. Every section below it is the artifact check's own procedure** — what *that* dispatch carries, its bar, its verdicts, its cap — and an instrument dispatching a cold seat for anything else takes its dispatch contract from its own cell. Reading on and applying what follows is how a consumer ends up holding the artifact it was supposed to be cold to.

## What the dispatch carries

**The draft artifact and the implementation brief it carries, and nothing else that aims attention.** The artifact's own test is that an implementer who was not there builds the agreed thing from those two alone. A seat given more settles the reading against material the implementer will not have, so the check passes documents that fail the very test it is run to apply.
...
**The working root, and the commit the seat confirms before anything else.** The artifact is judged on whether its reading holds against the tree, so the tree has to be reachable: what the dispatch bounds is what aims the seat's attention, never what it may read. Where the tree has no commit to name — one built for a run of its own commonly has no history at all — the dispatch says so and asks the seat to report what it finds instead, rather than sending an instruction no seat can satisfy.

**That root should be isolated — its copy of the text under judgement the change's own — and the dispatch states outright that any always-on text reaching the seat from elsewhere is not its governing text.** A fresh dispatch comes up on the repository default rather than the branch the work is on, every time rather than commonly, so a seat judging an artifact about a rule change reads the rule the change replaces and rules against it unless the dispatch names the revision and what lands it there — `git checkout --detach <sha>`, inside the root. Naming the root is not enough on its own: a root is not a revision, the stale text arrives beside it, and the statement is what lets a recipient reject it. **Diffing against the default branch does not establish it either**, a seat having been handed text from a tree that was neither the change, its base, nor that branch.
...
**The bar below, the three verdicts below, what a verdict must name, and the row-and-cell trace check.** The seat does not read this file, so a dispatch omitting the verdicts asks a three-valued question in a two-valued voice and gets what it asked for, one omitting the bar leaves the seat to infer a threshold, and one omitting the trace check leaves the seat without the implementation brief's completeness test. **So is the closed list under *What the draft carries between rounds*** — a seat that does not know the list is closed asks for a fifth thing, and additions of that kind have been made because a verdict asked for them.

## The three verdicts

The verdict **names the text it judged** — the sha256 and byte count it was given, and the commit it confirmed, or what it found where the tree had none — so a later reader knows what it covers. An adverse one **names the points it fails on**: the claim, criterion or choice, one line each, not the prose of the finding.

- **would** — the reading would deliver every non-`unchanged` reader cell of the implementation brief. This is the one that settles the artifact.
...
**The seat judges that third one against the implementation brief, because the issue is withheld from it.** A criterion turns on an answer the implementation brief does not settle, and what turns on the pick is a genuine fork under the test this cell states. That is runnable by a seat holding the artifact and the implementation brief, and it is the same defect seen from the party who will be handed the document. Where the artifact itself flags a question as open, the seat reads that as a candidate and rules on it the same way.

**A `not settleable` verdict is discharged by carrying the fork rather than closing it** — criteria that hold whichever way the call lands, the turning ones written against the recommendation and marked contingent. For this question about the affirmed work, the holder first takes any record or precedent question to the assigned coordinating session, or performs and records that check where none is assigned; it decides and records an inside-row call, and puts an owner ask only where the answer changes an affirmed row or the expected saving exceeds the stated cost of asking. **That discharge is its own route to settled, and it is the one revision touching a judged criterion that buys no fresh verdict**: the seat's finding was that no reading settles the artifact, so a revision performing exactly the remedy it named cannot fail the reading it never affirmed. It is not the reading amendment recorded elsewhere as a further comment — this one edits the artifact's criteria — and it is the single exception to the rule below, not an instance of it.

**Where the fork is one that could have made the implementation brief wrong, the implementation brief is amended instead.** Where the amended lane owes an artifact, that artifact is settled against the new term with a fresh dispatch, because the earlier reading judged a term that has since changed; an amendment selecting the mechanical lane owes no artifact or cold dispatch.

## What bounds the re-runs

**Two rounds, then the post-affirmation decision boundary, and the count licenses nothing:** after a **second adverse verdict** the artifact is still unsettled, and no third seat runs. Only `would not` counts toward it — `not settleable` settles the artifact by its own discharge, and a revision buying no fresh reading buys no round either.

**The implementer first repairs what a repair reaches** — a claim the verdicts found false about the tree is probed and rewritten, a passage of the wrong shape restated; those are the repeated-point rule's own remedies and no ruling substitutes for them. **The holder decides and records every unresolved point inside the affirmed rows.** A question about the affirmed work that the record or precedent can answer goes first to the assigned coordinating session, or through the holder's recorded check when none is assigned. The owner is reached only where the answer changes an affirmed row or the expected saving exceeds the ask's cost, through the one argued form with that cost stated. **The repairs and recorded cap resolution settle the artifact** — the label states **settled by the recorded cap resolution**, naming any qualifying owner ruling, never the word bare, so no stage inherits it as a reading a seat affirmed. **Against the repeated-point rule's third response the cap governs**: the work proceeds on the recommendation below the cap; at the cap, the holder records the resolution before the artifact can be built from. **An amended implementation brief whose lane owes an artifact is a new term, and the count starts again there** — that artifact is settled against it with a fresh dispatch, because two verdicts against the implementation brief it replaced judged something else. An amendment selecting the mechanical lane leaves this artifact process instead. Exhausted rounds alone are not a third owner door.

**Two things bound the loop below the cap.**

**The repeated-point rule (`../references/the-artifact.md`)**, whose third response is the fork above — read it there, with its decision citation, rather than twice.

**A revision that neither changes nor adds a claim does not buy a fresh reading.** The verdict named the text it judged; where a revision touches nothing that verdict turned on **and introduces nothing it did not cover**, the implementer states what it changed and which claim of the verdict's it did not touch, and returns it to the holder. **Adding is reached as surely as changing** — a new acceptance criterion or a new boundary alters no existing claim, so a carve-out keyed to change alone would let an unjudged criterion into a settled artifact under the verdict's own hash. Replacing the draft label with the settlement block is the paradigm, that label existing after a verdict rather than before it; **the block names the verdict, the text it judged, and what settled the artifact on the route it took** — the revision discharging a `not settleable`; the recorded resolution at the cap and any qualifying owner ruling; where the check could not be got at all, what was tried, what went unjudged, and why no qualifying party in reach could supply the seat, a dispatch withholding a qualifying check not being that case and leaving a draft; or nothing further where a seat said *would*. The round's history goes where the table below sends it.
...
**One line per point a verdict lands on, and that list is closed.** It is the whole of the provenance the round leaves in the artifact. Everything else a run of rounds generates is aimed at the checker rather than the builder, and has a home already:

| what accumulates | where it goes |
| --- | --- |
| a status line enumerating verdicts and revisions | the pull request body |
| a list of what N adverse verdicts changed | the pull request body |
