...
**After affirmation, an ask qualifies only when its answer changes an affirmed row or when the expected saving to the lab exceeds the cost of asking.** State which case applies and state the cost in this same argued form. A row-changing ask still states its cost without pretending savings are its reason; unknown cost or saving stays unknown rather than becoming invented arithmetic. A question the record or precedent can answer goes to the repository's assigned coordinating session first, or through the holder's recorded check when none is assigned. These are constraints on when the one form is used, not a second form.
