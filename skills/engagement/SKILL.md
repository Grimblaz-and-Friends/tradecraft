...
**It is settled in conversation, then posted.** What gets built is the owner's call, so the artifact is drafted and revised with him in chat. Until he affirms it you are holding a **draft**, and for nearly all of the work that is the only object there is. A draft is labelled as one, is complete enough to argue with rather than complete, and carries every question still open as open — the settled version is what the rest of this section is about, and a draft that reads like one has already taken a decision nobody gave it.

**Where the issue's own question is still open, the artifact carries the fork rather than closing it.** Its criteria are the ones that hold whichever way he rules; the fork goes to him in the argued form above, and any criterion that depends on his answer is written against the recommendation and marked as replaced if he rules otherwise. **An artifact whose criteria presuppose an answer the issue leaves open has closed a fork silently.** That is the mirror of asking where no fork exists, and the costlier of the two: the first spends his attention for nothing, this one takes a decision that was his and leaves no trace that it was ever open.

- **Acceptance criteria** are the change's success definition, and its review judges against them. Write each one so a reader can tell from the diff whether it is met — a criterion nobody can check is a wish. They state what must be true of the result, not what it may cost: a **forecast** — any figure about a tree that does not exist yet — belongs **outside the artifact**, in the PR body, the decision entry, or the review's report, each of which is written against a tree that does. A figure **measured** on a tree that exists is lawful here and is often load-bearing, because a fork is not arguable without the counts it turns on; it travels with the query or derivation that produced it, so a seat reaching it verifies rather than inherits. The whole artifact reaches every stage of the review, and that is a reason to make a figure checkable, never to exile one. [D-155]
...
**What the artifact carries about any check is its result, never the act.** A spike that fired carries what came back; a condition that did not fire carries the material you consulted instead — the enumeration, the counts, the file you opened. A sentence whose whole content is that you looked is not a result, and an artifact is judged on what it lets an implementer build.
