...
**Stand up an instance of the result as this change leaves it, and state the job in the product's terms rather than the repository's** — *do this thing the product is for*, never *exercise the screen this change touched*.

**Cut it off from everything an act inside it could reach.** Production data and the live board are the obvious two and not the list: the acts that leave a product isolation are the ones that reach a real person or a real system — a message sent, a payment taken, a write to shared storage, a call to a third party. **State the general form rather than a list**, because any list is short by the time it is written; the test is whether the act reaches outside the run. What replaces such a thing is inside the isolation, on the same principle the tree procedure applies to a remote: a stand-in the consumer can drive is what keeps runnable a job whose success *is* a notification.

**The dispatcher's three checks bind here, and the tree wording of two of them is not a scope limit.**

- **The instance holds the material**, which for a product means the change's own build and not a neighbouring one.
...
**What the note records for such a run is in the cell**, under the session note, in `the-note.md`: which build, and what that build was cut off from.
