...
**A change to how a later session must work, or to what someone using the result can do, buys a session on the built result** — a skill's behaviour, a mechanism's surface, or what a consumer can do with the running result. That last is the case a repository whose product is software meets on almost every change. Mechanical work — a typo, a behaviour-preserving dependency bump, a record append — does not, and still owes the line below saying none ran.
