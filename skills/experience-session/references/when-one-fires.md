...
**A change to how a later session must work, or to what someone using the result can do, buys a session on the built result** — a skill's behaviour, a mechanism's surface, or what a consumer can do with the running result. That last is the case a repository whose product is software meets on almost every change. Purely mechanical work — a typo, a behaviour-preserving dependency bump, a record append — does not. An owner-affirmed `ordinary` / `mechanical` lane likewise buys none, regardless of changed paths, because the affirmed brief has already judged the work a determinate correction with no substantive behavior choice left to test.
