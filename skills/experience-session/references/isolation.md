...
**`core.autocrlf` is disabled so the consumer's later Git operations in that repository — an archive, checkout or reset — emit the bytes the copy holds instead of applying an inherited conversion.** It does not make the extract byte-identical: attributes or configuration in the source can transform bytes while creating the archive, and no setting applied after extraction repairs them; the raw-byte comparison below is what finds that divergence.
...
The two object IDs must match for every file. If they do not, do not dispatch: change how that path is exported so the archive contains the committed blob bytes, because no setting applied after extraction can repair bytes the source's archive process already transformed.

**So inspect the extract, and inspect it as an operation.** Listing the tree catches a missing path and nothing else — a leak lives inside a file the dispatcher named and would tick off as present. Read the content: search the extract for the branch name, the issue and pull request numbers, the decision entry's slug, any word the change is about, and descriptions of the practice's own instruments or the consumer's isolated situation. **And check paths at the granularity that fails, not the one you typed** — `export-ignore` set below a named directory leaves the named path present and a file under it gone, so comparing the pathspec against the top-level listing passes while material is missing. Compare against what the job needs.
