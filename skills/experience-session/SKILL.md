...
`<outside>` is any directory not under the change's repository, and `git init` runs whether or not the job commits — git resolves upward, so a tree left inside the repo answers the consumer's first `git log` with the branch name and the fix batch's own commit subjects. Spell the `-f` path in the form your shell's `tar` accepts: GNU tar reads a leading drive letter as a remote host. A checkout, a clone, or a worktree hands over that branch name and those subjects outright.
