...
- **Where a step the job owed went unperformed — for want of the authority it needed, or because the dispatch put it out of scope — the note says which.** A truncated run is still a run; a truncation left unsaid reads as a complete one, and a run stopped by its own extent reads exactly like one the material never fought.
