...
| **causes #N** / **caused-by #N** | this produced that, or that produced this — the symptom is what the cause was observed doing, and fixing the cause may discharge it or leave it standing | rank the cause above its symptoms and work them as one group led by it; do not schedule a symptom on its own while its cause is open |
...
Each paired relationship is written in both directions; `blocks` exists because a filing that unblocks an existing issue would otherwise have to edit that issue to record the order.
