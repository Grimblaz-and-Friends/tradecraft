...
**The script sits beside this file at `scripts/pool.py`, and every path this cell writes is resolved against the directory of the file that names it** -- which is what makes one contract hold in an installed plugin and in this cell's own source repository alike, and is why a file in this cell's depth names the same script one directory up. A session sent here by another cell, running from somewhere else, prefixes the path to this cell; [sessions that did not have concluded the script does not exist](https://github.com/Grimblaz-and-Friends/tradecraft/issues/504).
