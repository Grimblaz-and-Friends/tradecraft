...
**The cause relationship is not a verb at all: it is GitHub's sub-issue link, the cause as parent, the parent carrying a `cause` label.** Set it when you file, and the label with it — the link means a task split into parts everywhere else, and the label is the only thing saying this one means causation. **It is exempt from the both-directions sentence above because there is nothing to write twice.** One link is carried by both issues, so a session opening a symptom sees its cause and a session opening the cause sees every symptom, whichever was filed first and without either body saying so. That is exactly what the verbs could not do: [fourteen of fourteen symptoms of one cause were silent about it](https://github.com/Grimblaz-and-Friends/tradecraft/issues/425), because writing the reverse meant one hand edit per symptom on issues a ranking may already have read. **GitHub refuses a second parent**, so a symptom carries one cause and the question of what two would mean cannot be asserted. A causal relationship you believe but cannot set — the cause sits in another repository, or you lack the access — goes in the tie block in prose and is flagged, the same way a relationship no name expresses does.
