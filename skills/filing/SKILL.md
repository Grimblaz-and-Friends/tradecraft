...
**The script sits beside this file at `scripts/pool.py`, and every path this cell writes is resolved against the directory this file is in** — which is what makes one contract hold in an installed plugin and in this cell's own source repository alike. A session sent here by another cell, running from somewhere else, prefixes the path to this cell. Two did not, both concluding the script does not exist; one published that absence into two draft filings before checking it. It does the mechanics, and the judgment -- what a rating should be, what the case against a candidate is -- stays the session's:
