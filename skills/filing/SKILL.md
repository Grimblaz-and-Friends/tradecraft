...
**The script sits beside this file at `scripts/pool.py`, and every path this cell writes is resolved against the directory this file is in** -- which is what makes one contract hold in an installed plugin and in this cell's own source repository alike. A session sent here by another cell, running from somewhere else, prefixes the path to this cell; [sessions that did not have concluded the script does not exist](https://github.com/Grimblaz-and-Friends/tradecraft/issues/504). `scripts/pool.py` does the mechanics, and the judgment -- what a rating should be, what the case against a candidate is -- stays the session's:
...
`scripts/pool-policy.json` carries the labels, the bands, the tie-break and the shortlist size; a repository overrides it with a file named `pool-policy.json` at its own root -- that one by its bare name, the only path here the rule above does not reach, and one that wants a wholly different approach says so in its own doctrine and does not run any of this.
