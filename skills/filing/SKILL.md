...
**Items the ratings leave equal are ordered by the policy's `tie_break`** — as shipped, more symptoms under it first, then most recently touched, with the issue number as the last resort once those are exhausted; `shortlist` says how many were tied at its cut and which key put the last one raised above the first one not, and says so when the keys separated nothing and the number is what chose. A repository that wants them the other way round swaps the two words, and one content with the issue number writes an empty list and gets that, declared. [#452]
