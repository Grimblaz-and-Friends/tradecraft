...
- **Before committing Python work governed by this cell, run its five guards with `python scripts/lint.py <repository-root>`**, resolving `scripts/lint.py` against the directory containing this file, so the contract reaches the same code from a source checkout or an installed plugin.
