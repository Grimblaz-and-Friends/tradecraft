...
The command refetches the head before and after publication. A move before the write requires recomposition; a move during the write leaves the older-head document in place and does not report current-head completion. When the effective classification requires no use, the same comment carries the generated current-head `no-use` compatibility carrier and its real reason. For an affirmed mechanical lane, the proof's source is the authorized affirmed comment and the reason is the lane exemption even when changed paths match the use policy; otherwise the reason comes from that policy. A bought use keeps its source head and original note rather than manufacturing a current-head copy.
...
- `policy` identifies the work configuration and use-policy path, repository revision and content digest used to compose the result.
...
- `floor.checks` retains check id, exact name, application, workflow and run identities where available, head, status, conclusion and timestamps. Different workflows sharing a display name remain different records; only genuine reruns within one workflow compete by start time and numeric check id.
- `use` distinguishes current-head, ancestor, generated and missing evidence. A generated mechanical exemption names its affirmed-comment source and reason. An ancestor use lists every intervening commit and both names of renamed paths; any use-bought path in any intervening commit makes it stale, including a change later reverted.
...
Posting proof does not certify readiness and may expose incomplete evidence. After a confirmed publication, the command requests a rerun only for an identified current-head gate workflow run carrying both run and workflow identities. A pending run is reported as pending; no run is selected from its display name alone, and submission is never reported as a pass. The returned JSON names the comment action and each run identity, request state and observed pending result.

## Reviewer readiness

`run ready-reviewers` validates the current-head floor and the same effective use classification proof uses, applies `.tradecraft/work.json`'s optional `reviewer_label`, then marks a draft pull request ready and verifies both results. A mechanical lane therefore requires its generated current-head no-use carrier, never a use bundle. Omitted or null `reviewer_label` means no label is declared. The label is applied before the ready transition so ready-triggered integrations can see it. On an already-ready pull request the command repairs a missing label without toggling draft status; after a partial failure, repeating the command finishes the remaining operation.
