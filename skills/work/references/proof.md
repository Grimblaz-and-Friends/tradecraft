...
- `floor.checks` retains check id, exact name, application, workflow and run identities where available, head, status, conclusion and timestamps. Different workflows sharing a display name remain different records; only genuine reruns within one runtime workflow compete by start time and numeric check id. A positively identified required gate leaves this ordinary set. With identified requirements, an Actions run with unresolved provenance waits under `unidentified`, while a run resolved to another source enters the ordinary floor. With unreadable rules, every Actions run could be the gate, so a red or pending Actions result waits under `unidentified` instead of routing to the floor.
