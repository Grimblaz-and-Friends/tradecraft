...
Before each launch, `run` maps the stage to `implementer`, `ordinary-seat`, `cold-seat`, `terminal-seat`, or `use-consumer`; reads that role only from the latest lawful whole-choice override line; and otherwise reads the selected launcher's standing default without changing it. A role omitted from that line, or whose entry names another vendor, takes the selected launcher's default rather than inheriting an older override. Model and effort, their separate sources, their scope, and every discoverable runtime executable are passed explicitly to the launcher. An unavailable executable remains an unavailable attempt rather than acquiring a requested setting as an observed one.
...
A merged implementing pull request on an open issue returns the holder-owned choice to close the issue or explicitly run another build. The latter is a fresh named build; the entrance does not infer it from the still-open issue.
