...
A merged implementing pull request on an open issue returns the holder-owned choice to close the issue or explicitly run another build. The latter is a fresh named build; the entrance does not infer it from the still-open issue, and it refuses while the merged implementation's registration remains active so the holder can `release` that registration first.
