...
It validates the adjacent metadata, source revision, detached neutral repository and carried bytes before launching the consumer through `dispatch_seat.py`. `run release-report` launches nobody and returns the report handoff to the holder; it refuses a custom dispatch. That handoff reports each required source and aggregates it as `green`, `red`, `stale`, `absent`, `none` or `unidentified`. Only `green` and the positively established `none` need no gate departure: another verdict directs the holder to restate the `**Path departures:**` paragraph with its reason, while unreadable rules are recorded as uncertainty rather than as a claim that a particular gate was bypassed. It records rather than forbids a departure, because merging remains the owner's decision. Repeating either the ordinary decision or the release-report handoff cannot buy another recipient.
