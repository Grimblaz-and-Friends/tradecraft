...
Power users may put one of `artifact`, `cold-seat`, `build`, `floor`, `use`, `review-disposition` or `release-report` before the options. That runs only the named stage and exits. The direct `release` and `adopt` commands below occupy the same positional slot, but neither is a stage or dispatches. Every builder prompt names one stage and tells the recipient to return rather than start another.
