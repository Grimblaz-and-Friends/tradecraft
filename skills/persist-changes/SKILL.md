...
- **When the push fails:** stop and read the reason line — it carries git's actual error. Divergence means fetch and inspect; auth, protection, or missing-remote errors mean the environment needs attention. Deciding how to integrate or recover is deliberately outside this script, because the right answer depends on context a staging tool cannot see.
