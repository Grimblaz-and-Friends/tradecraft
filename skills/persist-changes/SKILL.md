...
**Pause discipline: typed-halt** — the [constitutional statute](https://github.com/Grimblaz-and-Friends/tradecraft/blob/main/docs/architecture/constitution.md) §5's vocabulary, named so an audit conducted in the charter's own terms can find it. This skill never asks a question: every stop the script's own logic reaches is a typed one-line `not-persisted: <reason>`, so it is safe in unattended runs. The judgment section below is what you decide *before* invoking it and *after* it stops, never a question the skill puts to you mid-run.
