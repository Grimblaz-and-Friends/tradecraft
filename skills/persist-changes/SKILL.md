...
**Pause discipline: typed-halt** — ADR-005's vocabulary, named so an audit conducted in the charter's own terms can find it. This skill never asks a question: every stop is a typed one-line `not-persisted: <reason>`, so it is safe in unattended runs. There is no step outside that rule; the judgment section below is what you decide *before* invoking it and *after* it stops, never a question the skill puts to you mid-run.
