...
**Purpose:** make every governing document in the practice accountable to a stated job. **Audience:** any session creating or revising a skill or governing document, in this repo or a repo that adopts the practice. **Success:** every document it governs can say what it is for, who reads it, and what its review should judge it against — and contains nothing that fails that test.
...
- **Pin a decision entry's evidence to the commit it shipped at.** Where an entry cites a file for what its text said, write the commit beside the path: an entry freezes while the tree keeps moving, so an unpinned quotation goes stale silently and a later reader cannot tell whether the entry was wrong or the file changed under it. A bare pointer to living material is left unpinned on purpose — that one is repaired by whatever change moves the target, and pinning it would send readers to a snapshot when they want the thing itself.
- **An entry carrying counts carries the query that produced them.** A reader who can re-derive a number never has to trust it, and an entry freezes with its arithmetic inside — so the query is the difference between a figure a later session can check and one it can only inherit.
