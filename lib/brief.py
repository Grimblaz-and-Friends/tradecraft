...
def _has_decision_block(text: str) -> bool:
    ...
    for line in text.splitlines():
        stripped = line.strip()
        if (_pipe_decision_header(stripped) or row_heading.fullmatch(stripped)
...
def missing_elements(text: str) -> tuple[str, ...]:
    ...
    missing: list[str] = []
    if not _has_label(text, "Shape"):
        missing.append(ELEMENTS[0])
    if not _has_label(text, "Readers"):
        missing.append(ELEMENTS[1])
    if not _has_decision_block(text):
        missing.append(ELEMENTS[2])
    if not _has_label(text, "Not this"):
        missing.append(ELEMENTS[3])
    if review_lane(text) is None:
