...
CELLS = "skills"
# The second source, and the reason this script has a plural at all. A rule
# only this repository needs cannot live in a shipped cell -- that cell ships
# to every adopter -- so before this it had nowhere to go but the always-on
# doctrine, and the always-on surface grew by construction rather than by
# anyone choosing it. A repo-only cell is a cell in every respect the runtimes
# care about; it differs in living under a repo-only path, so an adopter never
# loads it. [#260]
REPO_CELLS = "docs/cells"
# In the order a reader meets them. A name may appear in exactly one: two
# sources offering the same name would make `expected` pick by tuple order and
# the loser's prose would load nowhere, silently, which is the failure mode
# `check_cell_frontmatter` exists to catch one level down.
SOURCES = (CELLS, REPO_CELLS)
CELL_FILE = "SKILL.md"
...
def _body(name: str, surface: Surface, owner: str = CELLS) -> bytes:
    ...
    **It does not claim the named file holds the whole cell**, because for most
    of the cells here it does not: a cell is a `SKILL.md` plus its `references/`
    depth, and the body is what indexes that depth. The line read `carries all
    of it` from the first shim until PR #560's review measured it false.
    ...
    **It names the source that owns the cell**, which is not always `skills/`:
    a repo-only cell lives under a repo-only path and an adopter never loads
    it. A body pointing every reader at `skills/` would send half of them to a
    directory the cell is not in. [#260]
    """
    return (
        f"\n# {name}\n"
        f"\nGenerated by `tools/roster.py` from `{owner}/{name}/{CELL_FILE}`, which is"
        f" the `{name}` cell's body, and indexes whatever depth sits beside it. Read that file now.\n"
...
def cell_sources(root: Path) -> dict[str, str]:
    """Every cell a runtime would load, mapped to the source that owns it.

    A mapping rather than a list because every caller that has a name then
    needs its source -- to read it, and to name it in a finding a reader can
    act on. Returning the list and recovering the source at each site would be
    the same lookup written once per caller, which is how two of them come to
    disagree.

    **Sorted by name across both sources**, not grouped by source: the roster
    is one surface to a runtime, and a reader comparing entries to cells reads
    one alphabet.
    """
    found: dict[str, str] = {}
...
def cell_names(root: Path) -> list[str]:
    """Every cell a runtime would load, from either source, sorted."""
    return list(cell_sources(root))


def duplicate_names(root: Path) -> list[str]:
    """Names offered by more than one source, sorted.

    Not a finding this module rules on -- `verify` does -- but the detection
    belongs beside the mapping that silently drops the loser.
    """
    seen: dict[str, int] = {}
    for source in SOURCES:
        for name in names_under(root, source):
...
def expected(root: Path, name: str, surface: Surface) -> bytes:
    ...
    **The frontmatter is the same on every surface and the body is not.** What
    ...
    source = root / owner / name / CELL_FILE
    data = source.read_bytes()
    block = frontmatter(data)
    if block is None:
        raise ValueError(
            f"{owner}/{name}/{CELL_FILE} has no parseable frontmatter to copy"
        )
    return block + _body(name, surface, owner)
...
def verify(root: Path) -> list[str]:
    ...
    **The shapes that are about a cell rather than a surface are reported once
    ...
    cells = list(sources)
    for name in duplicate_names(root):
        findings.append(
            f"roster: `{name}` is a cell under more than one of "
            f"{', '.join(SOURCES)}, so one of them loads nowhere and which "
            f"one is decided by this script's source order rather than by "
            f"anyone -- no command repairs this; rename or remove one"
        )
        ...
        for name in cells:
            ...
            if is_generated(target):
                findings.append(
                    ...
                    f"{sources[name]}/{name}/{CELL_FILE}, so every {surface.runtime} "
        ...
        for name in roster_names(root, surface):
            ...
            if is_generated(root / where / name / CELL_FILE):
                findings.append(
                    ...
                    f"any of {', '.join(SOURCES)}, so every {surface.runtime} session here loads "
                    f"a retired trigger -- run `python tools/roster.py --write`"
                )
    if not names_under(root, CELLS):
        findings.append(
