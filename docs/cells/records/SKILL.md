...
A row carries the date, the issue whose brief it is, `puts` and `amendments` and `corrected_before_posted` and `scoring_pass` from the affirmation record, `cold_seat` as its rounds, verdicts and route, and `reversed`, which is `null` at landing. **A reversal is a later append, never an edit** — what counts as one is not settled, and the session that appends the first is the one that has to argue it.
