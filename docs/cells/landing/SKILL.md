...
The PR body states `Closes #N`, or one line saying it closes none and why. **A change that fixes a cause disposes of every issue tied to it as a symptom** — a closing reference for each one the fix discharged, and one line each for the rest saying why it stands. **Read the symptoms off the cause's own sub-issue list**, which is complete whichever order they were filed in — one link is carried by both issues, so a symptom filed after its cause no longer leaves the cause untouched, and the walk over every open body that used to work around exactly that is gone:

```
gh api graphql -f query='{repository(owner:"Grimblaz-and-Friends",name:"tradecraft"){
  issue(number:N){subIssues(first:100){nodes{number state}}}}}'
```
 **Clear the `Blocked` on every symptom left standing**, here rather than at the next refresh, because the cause is about to close and be archived off the board with the reason its symptoms are held. **Each closing reference is a bare keyword and one number, on its own line**: GitHub parses neither an emphasised keyword nor a comma-separated list, and [PR #263](https://github.com/Grimblaz-and-Friends/tradecraft/pull/263) merged green having closed none of the five it named. A shipped-zone change bumps the plugin version.
