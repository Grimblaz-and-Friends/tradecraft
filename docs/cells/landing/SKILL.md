...
- **Your change may owe no brief, may carry an affirmed mechanical lane, or may deliver the truth of some text, and you want to know what that changes about the order above** → `references/where-the-flow-starts.md`: which step each skips and which it never skips, who decides each class, and the slice of build that precedes the artifact on a truth-of-text change.
...
For issue-backed product work, enter through the `work` cell; it reads the issue and pull request and runs the next stage this flow calls for. The stages remain: branch first (`main` refuses direct pushes) → settle the implementation brief where one is owed, then the artifact where its affirmed lane requires one, a truth-of-text change drafting its replacement text first → build → run the floor — `python -m pytest tools/tests skills lib/tests -q` and `python tools/lint.py` → commit → publish the branch and **open the pull request as a draft** → run the required experience session or carry the current-head no-use line → **mark ready and apply `reviewers`** → dispose every connected reviewer's comment in its thread. An owner-affirmed mechanical lane takes the no-use branch regardless of changed paths. A panel runs only when the owner buys its lane in the implementation brief, because review depth cannot depend on a later memory. A batch rewriting what the material instructs, or changing what someone using the result can do, buys one more experience session, or the line declining it.
...
**Not every change starts that chain at the implementation brief**, purely mechanical work having none to settle, an owner-affirmed mechanical lane having a brief but no artifact or cold seat, and a change delivering the truth of some text taking a slice of build before the artifact. Which work is which — the `charter` cell's convergence rule and the affirmed pair the owner selects — and why each is stated at all: `references/where-the-flow-starts.md`.
