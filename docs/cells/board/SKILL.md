...
**Nothing reaches this board that has not been framed**, and `sync` refuses to empty a populated board on an empty framed set rather than reading a setup mistake as a decision; `sync --allow-empty` is how an operator says the emptiness is real. **Two routes frame something and the shortlist is only one of them** — what the owner filed, or told a session to file, is decided from the start and arrives framed, so a refresh meeting a framed issue that went through no shortlist is meeting the ordinary case rather than a mistake. The `filing` cell owns that rule.
