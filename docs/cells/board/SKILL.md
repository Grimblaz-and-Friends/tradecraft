...
**Run the `filing` cell's `cycle` command as part of the refresh, and answer what it names.** Nothing else runs it, and nothing leaves the pool by the pull until somebody does: the shortlist refuses over an unassessed top, so its first move is blocked while the question goes unasked. The refresh is the moment the owner already triggers, which is why the asking goes here rather than into a second job nobody owns. **How many it names is that cell's policy, at `assessment.per_cycle`, and this cell states no number of its own**, so a repository re-pacing its pool changes it in one place. Ask why for each it names, then link it under its cause or `assess <N> --none`, and put the answers in the note. **A refresh that could not reach them all still runs the command and names which it left** — and owes nothing further for those. That is deliberately weaker than the crossing above, and the difference is what each mechanism does with an item nobody finished: `raise` marks a crossing, so one read and not carried through goes quiet, while an unanswered assessment is marked by nothing and stays in the set `cycle` draws from.
...
- **The reach line** — the charter's row from the pointer-reach block `python tools/lint.py` prints, with the figure the previous note carried beside it. It is the prose a session ends up holding after following the pointers out of the always-on surface, and nothing a reader of these notes sees carries it, so growth in it is discovered by measuring rather than read. The lint reports that block and ranks nothing; so does this line. **The first note under this rule has no predecessor and says so**, rather than leaving the second number blank. **It does not merge with the drift look**: that is an observation over what closed, derived from `gh issue list`, and this is a measurement over prose in the tree.
