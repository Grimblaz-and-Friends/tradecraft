...
**A cause and its open symptoms are one bundle, and the refresh reads it off the ties rather than re-deriving it.** Name the bundle for the cause, rank the cause above every symptom, and give each symptom `Blocked` while the cause is open, the reading rule above being unable to see `Bundle` or position. [D-415] The ties are the first element of an issue body, so `gh issue list --repo Grimblaz-and-Friends/tradecraft --state open --limit 1000 --json number,body` reads them all in one call, at the bound the transport's own open-set reads use; **a cause filed before the verb existed carries its symptoms as a prose candidate tie and groups the same way**, the refresh being judgment rather than a parser. The owner may rule a symptom worked on its own while its cause is open; that exception is his, and the note carries it in his words.
