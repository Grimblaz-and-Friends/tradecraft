...
- **The flow feeds a portfolio.** Once every in-flight change carries a live cost/value estimate, changes become comparable — within the lane structure the values declaration sets, since values-funded work competes in its lane and never head-to-head against the measurable — and "continue this" always implicitly means "instead of that." Prioritization is a standing consequence of evidence, not a one-time backlog ordering — and it reaches captured-but-unstarted work too: an item that will never start is killed by the same move, at the cheapest possible point.
