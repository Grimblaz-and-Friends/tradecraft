...
- **Integrate dissolves into increments.** Children integrate continuously; parent-level Integrate becomes *declaring done* — still a judgment against the parent's own emergent criteria, informed by parent-level Validate, never a formality — and staged exposure is satisfied structurally by shipping value all along, the parent's rollback being the portfolio's kill or rescope of what remains.
