...
| --- | --- | --- |
| `skills/experience-session/references/running-one.md` | `## Running one`, L51–L67 | 5,602 |
| `skills/experience-session/references/the-note.md` | `## What a note carries`, L31–L41, plus L69 | 1,964 |
| `skills/experience-session/references/when-one-fires.md` | `## When one fires`, L43–L49 | 1,873 |
...
- **Repairing `skills/experience-session/references/running-instance.md:3`'s bare `` `isolation.md` ``.** The bare form is this tree's prevailing convention for a prose mention of a sibling — `skills/authoring/references/routing.md` and `docs/cells/records/references/what-a-review-records.md` both use it — and it is not the path form `tools/lint.py` reads. Changing it would be a convention change, which is not this one.
...
**One line changes that no source line moved.** `skills/experience-session/references/running-instance.md:17` credited *"the cell, under what a note carries"* with the run-record rule, naming a section of the body by its heading — and this change is what removes that heading. `skills/experience-session/references/isolation.md:3`'s *"the isolation rule it serves is in the cell whose run it is"* is a cell-level credit and survives the rule moving within the cell, so it is left alone.

## Cost

The cell's own prose grows **24,147 → 25,872** characters by the lint's reach measure, **+1,725** — three titles, three load conditions, three index entries and one repaired locator, against no deletion. Every cell that points at `experience-session` carries the increase in its reach row. What the split buys is not what the cell weighs but what a session loads on a given firing, which is the same trade [D-263](D-263-2026-08-30-the-isolation-procedure-sheds-and-states-its-conditions.md) recorded for this cell's first shed, and recorded honestly: that shed left both the body and the cell total larger than they began. This one does not — the body falls 12,151 to 3,575 — and the total still rises.

## What the record cannot say for itself

`docs/recorded-findings.jsonl` carries 20 entries naming this cell, four of which cite a body line: one `SKILL.md:13` and three `SKILL.md:54`. The body renumbers from 69 lines to 29, so the three at `:54` now resolve past the file's end. **All four were already stale at `686a3bc`, and this change is not what broke them** — there, `:13` is the `running-instance.md` depth-index entry, where the entry citing it is about the Time-box bullet's unit list, and `:54` is a blank line, where the three citing it are about the extent paragraph's reason clause. Nothing maintains that record, so this is the only surface where it can be said. **No recorded entry's stated ground is falsified by this change**: the nearest, entry 108, gives *"prose on a cell already in net growth"*, and the cell's own prose still grows.
