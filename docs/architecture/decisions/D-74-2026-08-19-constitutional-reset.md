...
The statute (`docs/architecture/constitution.md`), the nine frozen ADRs, and decision entries D-53 through D-69 are a frozen archive from this decision forward: readable history, never binding. Rules that carried forward into the doctrine did so by restatement there, not by reference. The trial road, the trivial floor and its canonical rebuttal forms, the ledger vocabularies, the `owner-pending` machinery, and the fixed decision-entry skeleton end here without replacement.
