...
**The shed bought no headroom, and this entry twice claimed otherwise — first that it bought room, then with figures its own fix batch had already invalidated.** Both the body and the cell total end *larger* than they began, the total by more than double: `python tools/figures.py --cell skills/experience-session/SKILL.md --cell-budget N --base 76b29e3`, on the tree that lands this, is the derivation, and what to read is the total rather than the headroom, which means nothing for a cell absent from the budget map; the body is printed too, and comparing either against the base means running the same command on both trees. **So affirmed criterion 7 is not met.** Its literal half holds — the frequent consult loads no git procedure — and its stated payoff inverts, because the remedies this change's own review ordered onto the always-on surface cost more than the fence's text saved. What that consult no longer loads is a procedure it had no use for; what it loads instead is rules that bind it. That is a defensible trade and it is not the trade the word *room* describes.
...
- **Whether `AGENTS.md` should scope its own flow to a tree with a remote is filed as [#267](https://github.com/Grimblaz-and-Friends/tradecraft/issues/267) rather than answered.** This entry rejected that option on always-on cost, and the experience session is evidence the rejected option addressed the friction a consumer actually hit. The affirmed boundary bars the edit, not the issue.
- **What a revealed read-back breach does to the run** — void, rerun, or recorded — is filed as [#266](https://github.com/Grimblaz-and-Friends/tradecraft/issues/266). Section 5's rule detects and stops there, and settling the disposition inside this change would decide a rule it did not set out to decide.
