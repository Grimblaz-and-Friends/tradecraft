...
**The shed bought no headroom, and this entry twice claimed otherwise — first that it bought room, then with figures its own fix batch had already invalidated.** Both the body and the cell total end *larger* than they began, the total by more than double: `python tools/figures.py --cell skills/experience-session/SKILL.md --cell-budget N --base 76b29e3`, on the tree that lands this, is the derivation, and what to read is the body delta and the total rather than the headroom, which means nothing for a cell absent from the budget map. **So affirmed criterion 7 is not met.** Its literal half holds — the frequent consult loads no git procedure — and its stated payoff inverts, because the remedies this change's own review ordered onto the always-on surface cost more than the fence's text saved. What that consult no longer loads is a procedure it had no use for; what it loads instead is rules that bind it. That is a defensible trade and it is not the trade the word *room* describes.
