...
- **The deletion-timing bound this change wrote for an incident in its own conduct is now carried nowhere.** A first draft required the isolation to outlive the recipient's first return, after this session removed a cold seat's worktree while that seat was still resumable and the seat woke to find its tree gone. That sentence was withdrawn as collateral of restoring `dispatch.md`, and it is not re-sited: the shipped tree is byte-identical to base on the point, so no consumer is worse off, and re-siting it would grow `spikes` further to impose a judgement — when a recipient is *finished with* a tree — that nothing here retains. Distinct from [#311](https://github.com/Grimblaz-and-Friends/tradecraft/issues/311), which is the worktree's **base** rather than the timing of its deletion. Recorded rather than fixed, and recurrence is what would promote it.
