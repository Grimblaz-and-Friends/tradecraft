...
1. **A cell naming a sibling stops being a defect and becomes the prescribed move.** Every rule keyed to *a cell names no sibling* now reads the other way. The reserved form is unchanged, so no existing text becomes unlawful. **Two places restated the ban and are now false, and each is deleted rather than corrected in place**: `README.md`, which told an adopter at the front door that *"Skills never depend on each other sideways"*, and `skills/authoring/SKILL.md`'s pointer paragraph, which named *"the charter's exemption in both directions"* and called the description *"the whole always-loaded triggering surface"*. An exception has as many sites as the rule; where the rule was restated, the restatement goes and the pointer at the owning file stays.
