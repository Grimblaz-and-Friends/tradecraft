...
**A new rule in §5**, placed directly after the attended-lane question rule so that *argued* is defined immediately where the statute first demands it: a decision put to the owner carries the live options, each with its pros and cons, and the recommendation among them.
...
**The duplication is structural, not chosen**, exactly as D-53's `sustained` gloss is: delegation is unavailable in that direction because the wall forbids the skill citing `docs/`. It therefore ships with the maintenance rule that makes duplication safe, stated in the skill itself — **a change to the form lands at both sites in the same change.** The plugin version rises to `0.13.0` accordingly.
