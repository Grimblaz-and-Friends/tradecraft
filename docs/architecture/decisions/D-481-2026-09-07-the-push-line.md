...
**The first artifact argued this away with the fade and had the mechanism backwards.** It claimed a decline touches the issue, so the clock restarts and the item needs three quiet windows to fall out of contention. Measured against the shipped `effective()` at `fade.quiet_days: 30`, an item declined at `sev:4 urg:4` reads `urg 4` for days 0–29 — **so a restart is precisely what holds it over the line**, and the remedy was the disease: it would have re-raised on every refresh after an explicit decline, with no bound at all. A month is what the argument then claimed, and that figure is wrong for the same reason the rest of it is — the re-ask itself posts a comment and applies a label, so each one restarts the very clock that was supposed to end the asking. The figure was wrong twice besides: pushability ends after one quiet window, not three, and ninety days is when the item reaches the floor, a different event. **A durable mark is the only thing that answers this**, and it costs one label rather than the fade doing work it cannot do.
...
## Corrections the review made to this entry

**Two claims this change made about its own evidence were false when written, and are corrected here rather than left on the record.**

**The artifact's criterion 13 said each pin was verified red.** It was not true of two. One assertion read `"?" in out` over a whole listing, and `_header` supplies a literal `?` as a column heading on every listing the command prints, so it passed whatever the rows said — a pin that could not fail. Criterion 6's two source assertions were never mutated at all. Both are now pinned and both were demonstrated red: the assertion reads its own row, and the sixteen mutants the fix batch ran are each killed by the named test.

**Criterion 6's residue was closed by construction, not by chance.** The first draft did read `awaiting-owner` from the pool's policy, which is exactly the class D-441 decision 4 recorded; decision 5 below is that correction. What the review left standing was only the redness claim above, not the criterion.
...
**The cap's next stop is the owner and it was not taken. That was wrong, and the review is what established it.** The reasoning at the time was that none of the three surviving grounds was a fork the owner had to settle — a self-contradiction, a shape the tree already fixed, and a missing obligation — and that the charter calls asking where no fork exists a fabricated gate.

**All three texts that reasoning cited say the opposite of what they were cited for.** `skills/engagement/references/cold-seat.md` conditions the cap on a *"second adverse verdict"* and not on whether a fork exists. `engagement`'s stretch rule names the cap as *a scheduled exception* to not interrupting the owner, so escalating was pre-authorised and could not be the fabricated gate being avoided. And the same file closes the route actually taken by name: *that response has the work proceeding on the recommendation, and at the cap it waits, the artifact being unsettled and a draft not built from.* Under that sentence this change was built from a draft, and no edit here repairs it.

**The repository had already run the correct route, the same day.** `docs/settling.jsonl`'s row for #455 records two `would not` verdicts and `"route": "owner ruling at the cap"`, landed hours before this branch. The worked precedent was in the tree and was not followed.

**Where it stands.** The whole history is with the owner, with the split in decision 2 named as the one with two coherent answers, and three routes open to them: ratify the artifact as settled by their ruling at the cap, direct a fresh seat, or rule it rebuilt. The `settling.jsonl` row for this change is not written until they answer, its `cold_seat.route` having no truthful value before then.
