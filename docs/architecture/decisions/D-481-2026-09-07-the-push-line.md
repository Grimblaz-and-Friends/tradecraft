...
**All three texts that reasoning cited say the opposite of what they were cited for.** `cold-seat.md:45` conditions the cap on a *second adverse verdict* and not on whether a fork exists. `engagement`'s stretch rule names the cap as *a scheduled exception* to not interrupting the owner, so escalating was pre-authorised and could not be the fabricated gate being avoided. And `cold-seat.md:47` closes the route actually taken by name: *that response has the work proceeding on the recommendation, and at the cap it waits, the artifact being unsettled and a draft not built from.* Under that sentence this change was built from a draft, and no edit here repairs it.
