...
`:14` required a review to begin from *purpose, audience, and success criteria*; `:41`'s assignment enumerated *purpose statement* alone. A dispatcher building from the contract could satisfy it carrying only a purpose sentence, and the recipient would then judge fitness — `:18`'s trichotomy, *fit for purpose, fit once the named fixes land, or not fit* — against a criterion set it never received. **That pointer was wrong when this entry first published it**, as `:20`, and it was wrong in [#101](https://github.com/Grimblaz-and-Friends/tradecraft/issues/101) and in the affirmed artifact before that; both are append-only records the correction cannot reach, so the quotation travels with the number here to make the pointer self-checking. Raised by an external reviewer on PR #97 and sustained on independent grounds in that review's external-pass reconciliation, then routed because the enumeration is verbatim from the pre-change text and outside that change's affirmed boundary.

**#101 records that nothing in the shipped zone settles which reading is intended.** Reading every occurrence settles it. Before this change the term appeared three times in the shipped zone, all in this file — `:16`, `:41`, `:79`, the definition at `:14` being the fourth occurrence — and `:16` reads *"the audience the artifact's purpose statement names."* A bare purpose sentence names no audience. **The file already used the term in the wide sense; what was missing is that nothing said so**, and a dispatcher reading `:41` alone got the narrow one.
...
**Why an appositive and not a relative pronoun.** *"which goes negative"* would have been shorter and is the construction this file cannot afford. `:41`'s five versions in one day turned on exactly this: `each` reached only the predecessor outputs by nearest antecedent, `both` could not distribute over a non-pair, and `itself` could bind to `evidence standards`. D-97's remedy was to remove the pronouns rather than choose better ones. *"a term that"* binds by position, exactly as a relative pronoun would — `:65` calls both items terms, so the category selects neither. Its advantage over *"which"* is narrower than immunity: it leaves a categorial noun where a later editor would otherwise find a bare pronoun and be free to rebind it.
...
**Cost, measured by `wc -w` against the tree this PR's round-one fix batch produced (`133f4d5` as the base), and recorded here as a fact:**
...
**Two of those sixteen words are the `[D-113]` citation tokens**, one per line. No estimate was published before the bytes existed — see below.

**The table was first published pre-review, labelled as derived *after the fix batch* when no fix batch had run**, and its `:14` and file rows were then invalidated by the round-one fix the review ordered at that line. The figures above are the re-derivation `:58` requires against the tree the fix produced; the earlier ones were arithmetically exact and are superseded rather than corrected.
