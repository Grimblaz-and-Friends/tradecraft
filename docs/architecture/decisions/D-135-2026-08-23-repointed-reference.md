...
**The freeze admits one edit that changes no claim: repointing a reference whose target moved, to its current location.** Surrounding prose does not move and nothing is added or removed. Where the sentence quotes or characterises the target, repointing would leave it untrue, so it stays.
...
**Where there is nothing to repoint** — the target retired rather than moved, or moved and rewritten — the reference is recorded with its reason. Without that disposition the check reds with no lawful answer, and a guard that blocks lawful work fails as hard as one that passes unlawful work.

**The rule does not ship alone.** `tools/lint.py` resolves the path references and relative links a decision entry writes in backticks or a markdown link — not bare filenames or unbackticked paths, which claim nothing about where a thing lives. A reference is lawful four ways: it resolves, it is pinned to the commit it shipped at, it is in the baseline of what was already dead, or it is recorded as unrepairable since. The baseline may only shrink, and the guard enforces that rather than asserting it: a row whose reference resolves again is reported until it is removed.
...
- **`D-90:25`** is a path that was never in this repository; **`D-99:37`** also names an agents directory a spike created and did not commit. Naming it here without backticks is deliberate — a bare mention claims nothing about where a thing lives, and the guard fired on this entry's own draft when it carried the path instead.
