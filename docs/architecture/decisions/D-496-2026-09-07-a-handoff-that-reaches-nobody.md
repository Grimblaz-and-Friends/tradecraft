...
**The corpus check this entry said the second lander owed was run against the merged tool**, and the claim survives with its base moved. `git show 43adc93:tools/trial_intake.py` classifies every one of the 369 bodies identically to the merged tool but for #502, #503 and #504 — the three filed under this rule — and `STATED_PATTERN` fires on none of the 356 filed before the element existed. For the record, and because no evidence on either pull request covered it: **#489 alone moved eleven bodies** between `2c84f66` and `43adc93`, which is that change's effect on the trial's baseline and not this one's.
