...
**3. The heading is `**Provenance:**`, and the origin is its first word.** The heading was chosen because bodies here already write it and `tools/trial_intake.py` already looked for it — the cheapest reliable material the `authoring` cell's admission order asks for. **The first-word rule is what makes it a key rather than a description**: three of the 356 bodies filed before this change carried `**Provenance:**` and their first words were `both`, `originated` and `sustained`, none of which any classifier can sort on. **That census counts one heading form and the pattern accepts two.** `STATED_PATTERN` also reads `**Provenance.**`, which two further pre-change bodies write — #279 opening `PR` and #483 opening `Found`, the second being the body decision 2 quotes above — so five carried the heading in some form and none of the five opened with an origin. The widened branch is certified against the whole slice rather than against the census: **the pattern fires on none of the 356**, which are the bodies created before this change's own filings, the last of them #487 at `2026-09-07T21:14:34Z`. The line sites with the evidence, the tie block staying the body's first element, because a required element with no stated place is one every filer sites differently.
...
**An open sibling is mid-rewrite of the tiers this re-certifies.** [PR #489](https://github.com/Grimblaz-and-Friends/tradecraft/pull/489), PR #451's own review fix batch, is open against this same base and rewrites `tools/trial_intake.py`: it removes `PROVENANCE_HEADINGS` and the loop in `provenance_text` that this change's fallback still calls, and it moves `A/B run` and `design sitting` between the tiers. **So what this decision re-certifies is the tiers as they stand at this change's own revision, never whatever the merge of the two produces.** Whichever lands second conflicts in `tools/trial_intake.py` and `tools/tests/test_trial_intake.py`, collides on two append-only records, and owes the pre-change corpus check again against the merged tool — neither pull request's evidence covers that state.
