...
**Method**, recorded so a later session reconciles rather than guesses. Two populations, each named by the marker that defines it. **Plan artifacts:** issue comments whose first four lines carry a `<!-- plan-issue-N -->` marker — 192 comments across 189 issues; an artifact is such a comment that also carries a `## Plan:` heading — 181; its opening is the first non-blank block after that heading that is neither an HTML comment nor a `---` fence. **Affirmation records:** issue comments carrying an `<!-- open-for-work-affirmed-N -->` marker — 23 across 13 issues; the statement is the first contiguous run of `>` lines, counted with the quote glyphs stripped. The medians reproduce exactly under every extraction; the population counts move by a few percent with the width of the marker scan; and any *adorned-versus-unadorned* split is classifier-sensitive — 81% of openings begin with a plain character, 47% if bold anywhere disqualifies — which is why no such figure is load-bearing below.
...
- **The affirmed what-statement** — a separate record posted *before* the plan (8 of 8 on issues carrying both), blockquoted in **21 of 23**, opening with a bold one-sentence headline in 11 of those 21, and running 94–354 words. **This was the thing affirmed.**
