...
`{"dispatches": n, "subagent_tokens": n}`, scoped to the review — first seat dispatch to terminal ruling, cycles, defense and judge. Convergence rounds, the convergence cold seat, spikes and experience sessions are the change's cost, not the review's, and the row's subject is the review. **A commissioned pass is not counted**, which an earlier draft of this change got wrong: `after-the-fix.md` defines one as a review tool the session invokes itself, so it is a tool call rather than a subagent and returns no subagent tokens — counting it would put the row's two integers over different populations. **A comparison against the by-hand figures on #355, #143 and #296 must say which of the two it is comparing**, because several of those are change-wide; that is the honest limit of what this field lets #360 score.
...
**Ordered, first match governing**, which is what makes them a partition rather than three enumerations that overlap: `record` (this change's own paperwork) is tested first, because a decision entry and an index row live inside the repo-only zone and a zone test would swallow them; then `shipped`; then `repo` as the **residual**, so every site in the tree has a lawful label. **One zone, not two:** of the five `record` sites only those two are in the tree at all, and none of the five is or could be shipped material an adopter installs. Both properties were adverse-verdict repairs: revision 2 defined `repo` as "anything under `docs/`" and so labelled this change's own decision entry two ways at once, and left `README.md`, the dotfiles and the generated mirrors outside all three.
