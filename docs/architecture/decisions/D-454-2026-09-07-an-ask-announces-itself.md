...
**3. The query is scoped, and that was measured rather than assumed.** Run unscoped, `gh search issues --label awaiting-owner --state open --include-prs` returns other people's repositories; the count is deliberately not frozen here, since search results move and this entry cannot be re-derived. Repo-scoped with the label absent returns empty at exit 0, and with a label that exists returns rows. **A third arm was not run and an earlier draft claimed both were:** `--owner <account>` also defeats the noise and preserves one look across an account's repositories, which is closer to what the brief promised. Whether the shipped query should be owner-scoped is filed rather than settled here — `--owner` still narrows to one account, so it reduces the plural rather than removing it.
...
**The evidence, at the scope it actually holds.** [#428](https://github.com/Grimblaz-and-Friends/tradecraft/issues/428) measured seven briefs on issues 380–426: every one clean of the mechanical disqualifiers, four of seven carrying a content one. **An earlier draft of this entry, and of the shipped sentence, generalised that to the whole corpus, and the review falsified it** — over every brief posted to this repository two carry a mechanical disqualifier at the strict comment scope and fourteen once issue and pull request bodies are counted, across all four classes. So the mechanical half is *not* self-enforcing and the pass scores it too. What survives is narrower and is the rule's actual ground: opening the form before drafting does not prevent the defect, which is why the step is keyed to the finished draft.
...
**6. The declaration guard checks presence and never truth, and the bound is the same fact as decision 4.** Every pull request body carries a line beginning `**Waiting on you:**`; `tools/check_ask_declaration.py` refuses one without it in CI. **A guard that read the body to decide whether an unmarked ask is buried in it would be making a content judgement** — a grep for question-shaped prose would fire on nearly every body in this repository. **This bound does not rest on decision 4's corpus** and an earlier draft wrongly said it did: the affirmed brief settles it directly, rejecting a detecting check before it was offered.
