...
The affirmed brief's seven rows land as affirmed. The default instruments become the executable floor, every connected reviewer once per pull request, and one experience session — no panel, defense, judge, review row or findings append. A panel is **bought by asking** on a pull request rather than owed. A pull request opens as a draft and is marked ready only once the floor and the experience session have run, because "ready" is the reviewers' trigger and so must mean proven. An unfixed finding takes one of exactly **two** ends — fixed now, in this change where it caused the finding and otherwise in one immediate follow-up pull request carrying the rest together, or the release report's ask to the owner once, with its fix and its cost — and **no issue is created for work the owner has not bought**. Row 4 was affirmed with a third end, the board as somewhere an agent could put a follow-up unasked; the owner amended it out on 2026-09-19, before merge, after this change's own first use of it put four issues in front of him that he had bought none of. Deferred attention is attention, taken twice, and a queue of agent-proposed work is the 108-item hearing rebuilt behind a stricter door. The invariant is prospective by his ruling: the operative rule binds from landing, and the thirteen unbought issues that predate it are cleared by his answers in the release report.
...
- **An issue is no longer something an agent creates.** The owner's answer is what creates one, so an open issue is work he bought; a finding he declines is recorded on the work and creates nothing. This is the amended row 4, and it retires the `pitched` fate a bought panel's terminal stage used to have.
