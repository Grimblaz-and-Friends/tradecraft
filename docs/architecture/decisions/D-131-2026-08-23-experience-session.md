...
The affirmed artifact gave the cell two duties. The second — **arbitration**, D-96's pattern generalised into naming a falsifier and scheduling the next real occasion as its live test — is deleted, on the owner's call **after the review closed**, and the cell ships single-duty. **The panel's row in `docs/reviews.jsonl` therefore stands against the two-duty text, not this one:** every seat, the defense, the post-fix cycle and the external reconciliation read a cell with two duties, and the prose that replaced them was checked by one cold pass and a defense rather than by the panel.

**The reason is design fit, not evidence.** [D-132](D-132-2026-08-23-spikes-graduate.md) landed the same day and broadened the spike's trigger to any load-bearing premise, naming *a candidate wording for a rule* outright and carrying the reframe that governing prose is a mechanism whose executor is a reader. It also already carries the route for a premise a run cannot settle: the premise enters what you were writing as a declared assumption with the falsifier that would settle it. **What transfers is the trigger and the naming of a falsifier**, both of which `skills/spikes` states better than arbitration did — which is enough to make the two cells a duplicate on the job that matters, so the weaker copy goes. **What does not transfer is the booking.** Arbitration also scheduled the next real occasion as the live test; nothing in the practice now returns to a declared assumption to execute it, so that limb ships with no successor. Said rather than migrated, because `authoring` permits a reason to be dropped and not to be dropped silently. Where a successor belongs is `skills/spikes`, which this change's boundary forbids it to touch — routed to [#124](https://github.com/Grimblaz-and-Friends/tradecraft/issues/124)'s line of work.
