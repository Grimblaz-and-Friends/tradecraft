...
**Trigger:** a change to how a later session must work — a skill's behaviour, or a mechanism's surface. Mechanical work is out. **Both edges were argued and are recorded here because the trigger's width is the dial the instrument's whole cost rides on:** narrower — a skill's behaviour only — was rejected as excluding the class [#121](https://github.com/Grimblaz-and-Friends/tradecraft/issues/121) came from, since that defect lives in a mechanism's surface rather than a skill's behaviour; every-non-mechanical-PR was rejected as the ratchet the anchor issue warns against. **Declining costs one line from birth**, recorded where the change is; no session blocks a pull request by existing or by being skipped.
