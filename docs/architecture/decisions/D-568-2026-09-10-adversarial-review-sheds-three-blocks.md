...
- **Three of the eight evidence standards** — the test pin that must go red, deriving a correction from its source, and re-deriving a diagnosis against the tree a fix produces — join `skills/adversarial-review/references/after-the-fix.md` under a new heading. All three are keyed to fix time in their own words, and that file is the one their reader already opens by rule; its title and load condition widen from a batch that has landed to the writing of one, because that is when the arriving rules bind. **Its exclusion sentence moves in the other direction, and that is a meaning change this change makes rather than inherits**: *A review's seats, defense and judge do not need it* was true of the file before these three arrived, and left standing it would have told three stages they may skip rules the heading `## Evidence standards — every seat, every stage` had bound them to one revision earlier. It now excludes them from the rest of the file and not from the arriving section.
...
**The class is a clause whose sense depended on what sat around it**, not a clause containing a particular word — **and it runs in two directions**: a clause that moved while its anchor stayed, and a clause that stayed while its anchor moved. The first direction has three members, repaired below, found by two sweeps — once over the body's positional vocabulary, once per moved block by reading the block itself in its new file. **Neither sweep can see the second direction**, because both start from text that moved; this change's review walked it and found two members. `SKILL.md`'s *a retired lens* keeps its binding in the evidence standards while its definition leaves for `skills/adversarial-review/references/lenses.md`, and this change's own repair below leaves *the chosen lane* a term defined only in `skills/adversarial-review/references/the-shape.md`. Both are left standing and named here rather than repaired: each is one hop from an index entry the body carries on every firing, and closing either adds weight to the body this change exists to relieve.
...
**The body shed 1,757 characters and the cell as a whole gained 1,516.** Decoded UTF-8 characters, which is what `tools/lint.py` measures: the body 9,871 → 8,114, and the cell's own prose 51,765 → 53,281 — two new files, two new index entries, one amended index entry, one new section heading, the three repairs, and what this change's review bought on top of them. Both figures are measured at this branch's tip, after that review's fix batch; the first commit's message states the cell gaining 1,282, which was this figure before the batch rewrote three of the four files. **The growth is the point rather than a cost overrun.** A cell body is charged to every firing of the cell and its depth to none, so 1,516 characters added off the body buys 1,757 that six of the cell's eight reader populations no longer pay.

**The body renumbers from 51 lines to 38.** **How many entries of `docs/recorded-findings.jsonl` name this cell is a query and not a number here**, because this change's own review writes into that file: the command below answers it, and what matters is the arithmetic it does not change — the `skills/adversarial-review/SKILL.md:<line>` citations among those entries now resolve to different text, two of them, both `:41`, past the file's end.

```
python -c "import json,pathlib,re; rs=[json.loads(l) for l in pathlib.Path('docs/recorded-findings.jsonl').read_text(encoding='utf-8').splitlines() if l.strip()]; sel=[r for r in rs if 'adversarial-review' in r.get('artifact','').lower()]; print(len(sel), [m for r in sel for m in re.findall(r'adversarial-review/SKILL[.]md:(\d+)', json.dumps(r))])"
```

 Nothing maintains that record, so this is stated rather than repaired; what keeps those entries findable is that `artifact` names the cell, which is the durable subject whichever of its files holds the sentence. **This is not the first cell in the [#476](https://github.com/Grimblaz-and-Friends/tradecraft/issues/476) programme to strand entries that way, and how many splits did is a query here rather than a figure.** An ordinal written into a frozen entry freezes at the moment it was taken, and this programme has already shipped several that went stale between drafting and landing; the command below re-derives it over every split that landed before this one, each endpoint pinned by sha. **One entry sits outside everything that reaches**: `docs/recorded-findings.jsonl`'s own entry about entries a relocation strands keys its `artifact` to that file rather than to a cell, so no review's fetch of a cell's entries carries it, and no stage of this change's review was routed to it. The ruling recorded in [D-561](D-561-2026-09-10-records-sheds-three-sections.md) governs them all: a split that moves prose inside a cell's own `references/` does not trigger the decision log's mover-pays repoint permission, the owner having ruled that on 2026-09-10.

```
python - <<'PY'
import json, re, subprocess
splits = [("#546", "3e70b5f", "skills/filing/SKILL.md"),
          ("#559", "8fc15dc", "docs/cells/landing/SKILL.md"),
          ("#560", "e5b8897", "docs/cells/board/SKILL.md"),
          ("#561", "ccc45e3", "docs/cells/records/SKILL.md"),
          ("#565", "686a3bc", "skills/engagement/SKILL.md")]
show = lambda rev, path: subprocess.run(["git", "show", f"{rev}:{path}"],
    capture_output=True).stdout.decode("utf-8").replace("\r\n", "\n")
for pr, sha, path in splits:
    b, a = show(sha + "^", path).split("\n"), show(sha, path).split("\n")
    rec = [json.loads(l) for l in show(sha, "docs/recorded-findings.jsonl").splitlines() if l.strip()]
    cites = [int(m) for r in rec for m in re.findall(re.escape(path) + r":(\d+)", json.dumps(r))]
    moved = sum(1 for c in cites if c > len(a) or (c <= len(b) and b[c - 1] != a[c - 1]))
    print(pr, path, f"{len(b)}->{len(a)} lines; {len(cites)} citations; {moved} resolve differently")
PY
```

This change's own row is that same derivation between `686a3bc` and this branch's tip.

**Three recorded statements are falsified by Move 1, and they are named rather than repaired.** Each says that choosing a lane loads only this body: `docs/admissions.jsonl`'s row for #357, whose `outflow` reads *the only material a lane choice loads is this body -- a reference is opened after the lane is picked*; its row for #361, whose 937-character item is admitted *because a session choosing the lane loads only this body*; and [D-371](D-371-2026-09-04-the-review-runs-once.md), *the one-round shape must be readable when the lane is chosen, before any reference opens*. After this change choosing the lane **is** opening a reference, and the session note carried into this change's review reports it in use: the consumer's third file was `skills/adversarial-review/references/the-shape.md`. The clause admission #357 bought now sits in that file. Nothing maintains either record, so this is stated rather than repaired — and what it costs a later session is precise: [#328](https://github.com/Grimblaz-and-Friends/tradecraft/issues/328)'s question, whether the one-round clause may now shed, has to be read against a premise this change reversed, and those two rows argue the opposite of what they argued when they were written.
