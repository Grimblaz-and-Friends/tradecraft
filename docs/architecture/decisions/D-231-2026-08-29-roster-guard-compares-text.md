...
**The invisibility has a bound, and it is not decorative.** Git's `text=auto` refuses to normalise **any file containing a lone carriage return** — one bare `\r` anywhere disables the conversion for that whole file, and every CRLF in it is then committed verbatim. So "a CRLF entry and its LF twin are the same commit" is true only while neither carries a lone CR. This is not hypothetical: **this change's own first draft committed two literal control bytes into this entry's index row**, which is how the bound was found, and a repository-wide scan of all 118 tracked blobs found that file the only CR-bearing one. **Which composition is exposed is narrower than a first draft of this paragraph said, and the difference is the whole of it.** An entry that merely *carries* a lone CR is not forgiven: `matches()` returns False and the finding fires, exactly as ruling 3 below requires and as two tests pin. What is forgiven is the case where the **cell** carries the lone CR, so `want` carries it too and the entry is that cell's faithful CRLF copy — there the two agree as text, `matches()` accepts them, and git commits the CRLF verbatim. Measured over four compositions rather than reasoned:

| composition | `matches()` | finding |
| --- | --- | --- |
| CRLF entry, plus a stray lone CR | False | fires |
| LF entry, plus a stray lone CR | False | fires |
| all-CR entry | False | fires |
| **cell carries the lone CR, entry is its CRLF copy** | **True** | **silent** |

That last row is the exposure, it is not closed here, and it is filed as [#234](https://github.com/Grimblaz-and-Friends/tradecraft/issues/234)'s neighbour in the byte-surface tranche of [#233](https://github.com/Grimblaz-and-Friends/tradecraft/issues/233). An earlier draft of this sentence said the guard forgives any lone-CR entry, which is a false negative in the direction that would make a reader distrust two passing tests -- this entry's own recurring defect, stated wider than the population measured, at its fifth site.
...
**4. The tree-level test stops asserting the entries are LF on disk.** That assertion was the wrong instrument for what it wanted. What the generator writes is pinned deterministically in `tmp_path` by `test_the_generator_introduces_no_carriage_return_of_its_own`, where no later copy can move it; asserting it again against the working tree asserted a property of whatever produced that tree, which is not this repository. The residual is that nothing checks the *committed* roster is LF. `tools/lint.py` had no carriage-return check of any kind when that was written; this change then added **check 19**, `check_docstring_control_chars`, which closes the half a byte scan cannot see — a control character in a docstring's *compiled* value. The committed-**byte** half is still unguarded and is [#233](https://github.com/Grimblaz-and-Friends/tradecraft/issues/233)'s kept tranche. The two halves are disjoint: Python's tokenizer folds a lone CR in source to a line feed before a docstring is compiled, so check 19 cannot see one on disk, while every other raw control byte it can.

**5. The substrate cell's third text-mode rule is not amended, and neither is `AGENTS.md`.** The cell's warrant paragraph, which says this defect class has never fired, is falsified by this change and is filed as [#235](https://github.com/Grimblaz-and-Friends/tradecraft/issues/235) — editing it is a shipped-zone change the affirmed boundary excluded by name. The rule says a file that will later be compared, restored or measured is *written and read as bytes*; `roster.write` still writes bytes and still has its guard. What this decision separates is the **write** half from the **compare** half, at one site where the file leaves this repository's control between the two. That is a bound on the rule's reach rather than a change to it, and it is stated in `tools/roster.py`'s module docstring — under the heading *"Written as bytes, compared as text"* — which then routes a reader down to `matches()` for the warrant.
