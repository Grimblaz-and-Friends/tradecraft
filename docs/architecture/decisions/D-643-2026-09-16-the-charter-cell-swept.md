...
| --- | --- | --- |
| `skills/charter/SKILL.md:27` — *"one that altered how a later session must work or what someone using the result can do"* | sharpen | **To another cell.** The trigger belongs to the `experience-session` cell, whose depth states the operative test at `skills/experience-session/references/when-one-fires.md:5`. Its description at `skills/experience-session/SKILL.md:3` names three limbs, but those limbs do not reach a change confined to an adopting repository's own root doctrine; the charter therefore names the cell rather than its description. The charter keeps the merge-time anchor and both outcomes: use the result, or write the declining line. |
...
**3. The experience-session trigger is that cell's.** The operative test is at `skills/experience-session/references/when-one-fires.md:5`. The description at `skills/experience-session/SKILL.md:3` names three limbs, but none reaches a change confined to an adopting repository's own root doctrine; the charter therefore names the cell rather than its description. The charter keeps what only it supplied: the before-merge anchor and the two outcomes.
...
**5. The out-of-review decline stays at `skills/charter/SKILL.md:33`.** It fires when no review cell has loaded. The copy quoted by the findings existed at `skills/adversarial-review/references/arbitration.md:15` at `bd71c4f`; it is absent now. Current `skills/adversarial-review/references/arbitration.md:19` carries both the role-separation sentence and the different rule requiring an independent ruling for a lapse outside review; `skills/adversarial-review/references/arbitration.md:27` at `bd71c4f` is the older role-separation phrasing. If #572 gives the review cell both the general decline rule and a route to readers outside a review, the charter line becomes the copy to remove; this change does not pretend that route already exists.
...
## The fix batch's outflow look

Because M1 edits the always-on charter, the look required by `skills/authoring/references/routing.md:31` read `AGENTS.md`, `CLAUDE.md`, the charter body, and every cell name and description under `skills/*/SKILL.md` and `docs/cells/*/SKILL.md`. It found no additional rule with a cheaper home: this fix deletes the false narrowing from an existing pointer to the `experience-session` cell, adds no concept or rule, and leaves the three description limbs at `skills/experience-session/SKILL.md:3` and the operative test at `skills/experience-session/references/when-one-fires.md:5`. The deletion is itself an outflow, so it discharges the obligation rather than incurring a matched removal.
...
- `skills/adversarial-review/references/arbitration.md:19` carries both the current role-separation sentence and the lapse-ruling sentence; `skills/adversarial-review/references/arbitration.md:27` at `bd71c4f` is the older role-separation phrasing for #572.
