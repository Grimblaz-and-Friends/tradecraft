...
**Each of those is a hole a version of this check shipped**, and none was anticipated — the module alias was found by three seats and both external reviewers, `stdin=None` and `input=None` by two seats, the `getoutput` family by three, and `check_output` and the positional forms by the post-fix look. **A completeness claim about a guard's own bounds is what turns a gap into a false all-clear**, and this entry made that mistake twice: the first version's docstring claimed *"Two consequences follow and are the guard's stated limits"* and four spellings escaped; the second announced it was not stating a closed list and then stated one of two items, which positional arguments and `*args` escaped. **The bound is now stated as what is read**: the callee and its keyword arguments. A second positional argument, a splat of either kind, and a non-literal `capture_output` are unread, and unread is silence — whether a stream is redirected is genuinely unknown there, and reddening on it blocks lawful work. **A stream keyword whose value is not the literal `None` is read as a redirect, including a name that happens to be `None` at run time** — a permanent bound of a call-site check, since nothing at the syntax tree knows what a name evaluates to.

**That makes three wrong statements of this one bound, which is the entry's most repeated mistake.** The first was a two-item list four spellings escaped. The second announced it was not a list and gave one of two, which positional arguments escaped. The third — the fix for the second — generalised *not a literal* from `capture_output` to every argument, and was false of **all 17 launch sites in this repository**, every one of them `stdin=subprocess.DEVNULL`, a non-literal the guard reads. Each version was written to correct the one before it. What finally holds is a statement of what is read rather than of what is not.
...
A description is an always-on surface, so the edit owes an outflow. **It is refused, and this is the refusal.** `skills/authoring/references/routing.md` directs the search over the **whole surface** rather than over the file that grew — `AGENTS.md`, the shipped charter, and every cell's name and description — so that is where it was run, and what follows is what it found. Inside the `substrate` description itself no move applies, for one structural reason: each move relocates *a rule*, and a description carries a name, a summary, and triggers rather than rules.
...
**The surface-wide look, and what it returned.** `AGENTS.md` was read against the three moves; the closest candidate is its record-keeping line, whose second clause names `docs/recorded-findings.jsonl` — a file this change appends to four times and whose obligation `the-record.md` already carries in full, so it is a live candidate for the *rule a guard now holds* move. It is **not taken here**: nothing guards that append today, the clause is two days old and arrived with [D-230], and moving a rule out of an always-on surface to pay for an unrelated edit is the *bad move to pay for one* that `routing.md` names in the same paragraph. The shipped charter and the other eight descriptions were read and offered nothing cheaper. So the surface grows by the difference between two runs of `python tools/figures.py` — on this PR's tree and on a worktree at `916af5d` — differencing the *from this practice for an adopter* value on the `always-on surface` line. That is the outflow applied and returning nothing, which `routing.md` distinguishes from the outflow skipped.
