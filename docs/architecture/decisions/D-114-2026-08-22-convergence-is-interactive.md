...
What the artifact contains, the before-the-first-commit timing, and the mechanical-work exemption are verbatim. **The affirmation-record clause was not**, and the artifact's own *"everything else … is verbatim"* line was wrong about it: implementation dropped the locative, so the bullet as affirmed, and as first landed, read *"The session records the affirmation, naming that comment"*. Four review seats filed it. The owner — who had been told that clause was untouched while the block in front of him reworded it — decided on 2026-08-23 to restore it, and the landed bullet reads *"The session records the affirmation **on the issue**, naming that comment"*. That preserves the pre-change locative, so the three meaning changes above are still the whole list against the old text; what it costs is the one place this change deviates from its own affirmed handoff contract, +13 characters against the affirmed proposal. The deviation is recorded at [#85](https://github.com/Grimblaz-and-Friends/tradecraft/issues/85#issuecomment-5383920241) and in the comment that answers it. The `convergence gate` naming is kept and re-attached: the gate is the conversation, not the comment's review.
...
- **Stating the handoff-contract bar in the doctrine as well.** It would bind without the cell being loaded. Rejected on `authoring`'s own *"never duplicated prose that can drift"* — the same line that made [D-104](D-104-2026-08-22-engagement-cell.md) delete the doctrine's copy of the plain brief's elements after that copy drifted before it even landed — and on budget: `AGENTS.md` lands at 7,419 of 8,000 characters — 581 to spare after the restoration above — and #86 is the next claimant on the remainder.
