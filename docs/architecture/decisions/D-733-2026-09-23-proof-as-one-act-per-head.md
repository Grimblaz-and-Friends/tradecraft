...
### Release reporting binds the gate verdict to the merge head

The owner [added the release-proof row after the fifth cross-change read](https://github.com/Grimblaz-and-Friends/tradecraft/issues/725#issuecomment-5804459089): release proof names the required gate verdict at the head that merges, and a red, stale or absent verdict makes the path-departures paragraph record the bypass and its reason at that head. The same ruling authorizes this builder resume before the new floor and use, without revising the pre-implementation artifact or buying another cold settlement.

The proof document cannot carry the verdict of the gate that verifies it. The holder-owned `run release-report` step therefore reports `green`, `red`, `stale` or `absent` for the current head, together with every latest gate run the entrance identified. For a non-green verdict it directs the holder to restate the `**Path departures:**` paragraph at that head with the bypass and its reason. This is evidence for the owner's merge decision, not a prohibition on that decision.
...
The [experience-consumer correction](https://github.com/Grimblaz-and-Friends/tradecraft/issues/725#issuecomment-5804354693) makes the shipped instruction agree with `run use` and the owner's ruling: the consumer takes the cold classification and Claude `max`, because its use run needs unbriefed perception while operating the result.
...
The owner [amended the Codex follow-up to keep Sol 6 in the comparison](https://github.com/Grimblaz-and-Friends/tradecraft/issues/725#issuecomment-5804292190). The Steward filed both runs as [#737](https://github.com/Grimblaz-and-Friends/tradecraft/issues/737), not on PR #733:
