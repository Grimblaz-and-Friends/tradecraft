...
**`skills/charter/SKILL.md` is deliberately absent from the map.** Its body is a term in every always-on row, so `check_always_on_budget` sizes it, and a ratified test pins it out of the body map as a smuggled second limit. That is also the standard's one stated exception, and it is stated with the reason: **shedding to `references/` cannot relieve an always-on row**, so the move this standard teaches is the one move that does not work there. Stating only the cap would send a writer to a remedy that does not apply.
