...
4. **No meta-governance.** No admission contracts, no ledgers of the store's own history, no sweep instruments, no budgets. The inbox is plain notes and triage is a glance, because an entry is either landed soon or deleted: an entry that survives a few triages untouched has failed the pickup test (ADR-006) and is deleted, not curated. **If the inbox ever seems to need governance, that is the signal that exits have gotten expensive again — fix the exits, don't govern the pool.**
