...
- **Then, without being asked: push the PR, run the review, reconcile the external comments** — in that order, so the automated reviewers run concurrently with the panel. The review is a check, never a question; handing it back as a command for the human to run invents a gate the charter does not have. (ADR-006 §3)
