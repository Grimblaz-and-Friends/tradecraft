...
- **Release.** Merging is the owner's, never the agent's. CI labels and comments on any PR touching this file, flagging it for the owner's specific review — not `CODEOWNERS`, which fires by requesting a review and so cannot reach the author of the PR, which here is always him. [D-80]
