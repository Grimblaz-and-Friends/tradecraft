...
- **Release.** `ci.yml` flags any PR touching this file, `CLAUDE.md`, the `charter` cell or a repo-only cell — each entire — or any other cell's name and description, a rename out of any of them included, for the owner's specific review. **Other shipped cell bodies are not flagged**: their rules arrive when the cell fires. [D-81] [D-394]
